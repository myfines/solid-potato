...
export async function buildMotorProject({client, name, projectDirectory, backupPath, deviceName='PLC_1', orderNumber=defaultOrderNumber, call, report=()=>{}}) {
  ...
  report('running','工作流步骤：生成并编译 Motor_Starter SCL'); const sclStarted=Date.now(); await call('tia_apply_scl',{project:activeProjectPath||`${projectDirectory}\\${name}.ap20`,deviceName,sourceName:'Motor_Starter_FB_DB_Source',filePath:`${projectDirectory}\\${name}_Motor_Starter_FB_DB.scl`,content:fbSource,overwrite:true}); report('success',`工作流步骤：Motor_Starter SCL 完成（${Date.now()-sclStarted}ms）`);
